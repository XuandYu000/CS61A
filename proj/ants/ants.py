...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if self.can_contain(place.ant): #can_contain函数在Ant类中已实现
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        now = self.place
        cur = 0
        while now is not beehive:
            if len(now.bees) != 0 :
                if cur >= self.min_range and cur <= self.max_range: 
                    return rANTdom_else_none(now.bees)
            now = now.entrance
            cur += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        def reflected_damage(amount):
            ...
            for bee in self.place.bees.copy(): #直接在原数组上修改会导致可能有些元素没有遍历到，因为reduce_armor原函数是会消除对象的，此时后面的会向前进一位导致有的元素可能不能遍历到
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        def Ninja_damage(amount):
            remain_bees = []
            for bee in self.place.bees:
                if bee.armor > amount:
                    remain_bees.append(bee)
            for bee in self.place.bees.copy():
                Insect.reduce_armor(bee,amount)
            self.place.bees = remain_bees
        Ninja_damage(self.damage)
        # END Problem 7
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant is not None: #在每一轮中受到由受保护的ant进行反应
...
# END Problem 13
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 13
    queen_deployed = False # 类属性定义蚁后是否已放置
    implemented = True   # Change to True to view in the GUI
    # END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.imposter: # 假货直接死亡
            Ant.reduce_armor(self, self.armor)
        else:
            super().action(gamestate) #对局作出反应
            curr_place = self.place.exit
            while curr_place is not None: # 对身后的蚂蚁增幅
                if curr_place.ant and not curr_place.ant.buffed:
                    curr_place.ant.damage <<= 1
                ...
                if isinstance(curr_place.ant, ContainerAnt) \
                    ...
                    and not curr_place.ant.contained_ant.buffed:
                        curr_place.ant.contained_ant.damage <<= 1
