...
def piggy_points(score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 2
    #掷0次所得的分数应该是对方分数的平方的最小位数加3
    return find_min_digit(score**2)+3

def find_min_digit(score):
    """Return the min digit of the score"""
    if score<10:
        return score
    return min(score%10,find_min_digit(score//10))
...
def more_boar(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    #这里的规则是如果玩家的分数的最左边两位数小于对方的最左边两位数，则可以获得额外的回合
    #但是前提是两个玩家的分数位数相同，位数较小的补前导0
    temp=max(player_score,opponent_score)
    cnt=0
    while temp>=10:
        temp//=10
        cnt+=1
    left_most_p,left_second_p=player_score//(10**cnt),player_score//(10**(cnt-1))%10
    left_most_op,left_second_op=opponent_score//(10**cnt),opponent_score//(10**(cnt-1))%10
    return left_most_p<left_most_op and left_second_p<left_second_op
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if not more_boar(score1,score0):
                who=next_player(who)
        say=say(score0,score1) # 注意say在这里使递归调用的，如果只是say(score0,score1)那么每次都会创建一个新的函数，不能达到递归的效果。发现bug5分钟，debug 1小时啊 连这种递归都没想明白
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        ...
        if who==0:
            ...
            score=score1
        higher=score-last_score
        if higher>running_high:
            print(f'Player {who} has reached a new maximum point gain. {higher} point(s)!')
        return announce_highest(who,score,max(running_high,higher))
    return say #python高阶函数的用法，这是一个很好的应用
    # END PROBLEM 7
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if score>=GOAL_SCORE:
        return 0
    elif more_boar(score,opponent_score):
        return 0
    else:
