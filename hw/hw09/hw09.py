...
def in_order_traversal(t):
    ...
    if t.is_leaf():
        return [t.label]
    res = []
    res += in_order_traversal(t.branches[0])
    res.append(t.label)
    res += in_order_traversal(t.branches[1])
    return res
...
def interleaved_sum(n, odd_term, even_term):
    ...
    def helper(now, fla):
        if now == n + 1:
            return 0
        if fla == 1 :
            return odd_term(now) + helper(now + 1, fla * (-1))
        else:
            return even_term(now) + helper(now + 1, fla * (-1))
    return helper(1,1)
...
def mutate_reverse(link):
    ...
    while ptr is not Link.empty:
        values.append(ptr.first)
        ptr = ptr.rest
    ptr = link
    while ptr is not Link.empty:
        ptr.first = values.pop()
        ptr = ptr.rest
