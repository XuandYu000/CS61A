...
def g_iter(n):
    ...
    if n<=3:
        ...
        g1,g2,g3,i=1,2,3,3
        while i<n:
            g1,g2,g3,i=g2,g3,g1*3+g2*2+g3,i+1
        return g3
...
def missing_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    be=n%10
    def dfs(now,last,res):
        if now==0:
            return be-last-res+1
        # print(now,last,res)
        if now%10==last:
            return dfs(now//10,now%10,res)
        else:
            return dfs(now//10,now%10,res+1)
    return dfs(n,-1,0)
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_using(min_coin,total):
        if total==0:
            return 1
        elif total<0:
            return 0
            ...
            return count_using(min_coin*2,total)+count_using(min_coin,total-min_coin)
...
def make_anonymous_factorial():
    ...
    return lambda n:reduce(mul,range(1,n+1)) #reduce函数将后面的范围内的数应用到前面函数中
