...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            temp_edits, temp_cost = helper_align(skeleton_idx + 1, code_idx + 1) 
            final_edits, final_cost = str(skel_char) + temp_edits, temp_cost
            # print("DEBUG: ", final_edits,final_cost)
            possibilities.append((final_edits, final_cost))
        # Insert
        temp_edits, temp_cost = helper_align(skeleton_idx, code_idx + 1)
        final_edits, final_cost = "+[{}]".format(code_char) + temp_edits, temp_cost + 1
        possibilities.append((final_edits, final_cost))
        # Delete
        temp_edits, temp_cost = helper_align(skeleton_idx + 1, code_idx )
        final_edits, final_cost = "-[{}]".format(skel_char) + temp_edits, temp_cost + 1
        possibilities.append((final_edits, final_cost))
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    # 类似于整数分割的思路
    def helper(fir_sum, sec_sum, lst):
        if not lst:
            dif = abs(fir_sum - sec_sum)
            if dif < 0 or dif > d:
                return 0
            else:
                return 1
        return helper(fir_sum + lst[0], sec_sum, lst[1:]) + helper(fir_sum, sec_sum + lst[0], lst[1:])
        
    return helper(0, 0, s) >> 1
