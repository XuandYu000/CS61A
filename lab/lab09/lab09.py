...
def has_cycle_constant(link):
    ...
    """
    "*** YOUR CODE HERE ***"
    p_slow, p_fast =link, link #如果有循环一个一次走2步，一个一次走1步，某时刻必然会多走出一个循环此时p_slow 和 p_fast 相等
...
def reverse_other(t):
    ...
    if t.is_leaf():
        return 
    label_list=[]
    for b in t.branches: #将相邻两层有联系的看成一个整体，此时第二层需要交换
        label_list.append(b.label)
    for b, new_label in zip(t.branches,reversed(label_list)):
        b.label=new_label #交换第二层标签
        for bb in b.branches: #第二层继续往下一层继续递归， 这样做到隔行交换
            reverse_other(bb)
